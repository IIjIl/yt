    ...
    if cb_data.startswith("ytdata||"):
        ...
        if media_type == 'audio':
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "ملف صوتي 📼", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("ملف صوتي (محسن) 📼",
                                                                                                    callback_data=f"docaudio||{format_id}||{yturl}")]])
        else:
            buttons = InlineKeyboardMarkup([[InlineKeyboardButton(
                "مقطع فيديو 🎞", callback_data=f"{media_type}||{format_id}||{yturl}"), InlineKeyboardButton("ملف فيديو 🎞",
...
async def catch_youtube_dldata(c, q):
    ...
    await q.edit_message_reply_markup(
        InlineKeyboardMarkup([[InlineKeyboardButton("يتم التحميل انتظر . .", callback_data="down")]]))
...
async def send_file(c, q, med, filename):
    ...
    try:
        await q.edit_message_reply_markup(
            InlineKeyboardMarkup([[InlineKeyboardButton("جار التحميل . .", callback_data="down")]]))
